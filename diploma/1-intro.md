...
TLA+ - это язык формальной спецификации, который основан на нетипизированная теории мн-в, логике первого порядка и временной логике (TLA). TLA - это темпоральная логика, с помощью которой можно описать распределенные и многопоточные системы/алгоритмы.
...
TLA+ использует LTL - это логика линейного времени. Она хорошо подходит, так как свайства в ней формулируются для одного выполнения системы. Это хорошо описывает распределенные системы, так как свойства, которые описывают несколько выполнений сразу, не возникают в распределенных системах. Есть альтернативный подход - CTL - логика ветвящегося времени. Который не применим к распределенным системам.
