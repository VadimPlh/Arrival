...
В TLA+ рассматриваются траектории с бесконечным кол-вом состояний, это хорошо подходит для описания распределенных систем, так как система реактивна.
...
Этот метод тестирования помог обнаржуть сложные баги в DynamoDB, S3. Например, найден баг в DynamoDB, который требует 35 шагов алгоритма, он мог привести к потери данных при определенной последовательности шагов сбоев и восстановлений узлов.
...
4) Сама репликация устроена с помощью отказоустойчивого лога, который использует реплики. Лог - это последовательность команд (insert, merge, e.t.c). Лог хранится в ZooKeeper-е. Сами данные в записях лога не хранятся, их реплики получают с помощью взаимодействия по http.
