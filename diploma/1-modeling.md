...
Например: алгоритм не SI гарантирует Serializability. Для этого мы должны проверить, что конкурирующие транзакции можно упорядочить (они не образуют цикл) и не нарушаются инварианты алгоритма на запись и чтение. Например, что если две транзакции конкурируют, а их write set-ы пересекаются, то применится только одна из них у которой временная метка меньше, а другая будет отменена.
