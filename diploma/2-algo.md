...
Paxos/Raft: В этих алгоритмах основными участниками являются ноды. Мы хотим описать алгоритм общения между ними, моделируя передачу сообщений, сеть и т.д. Это самый высокий уровень детализации. Мы хотим полностью описать, как ведут себя участники алгоритма, как они решают проблему отказоустойчивости. В системах, где одной из составляющих является обмен сообщениями по сети надо уметь моделировать асинхронность сети.
...
Следует выделить основных участников системы. Ими могут быть ноды (Паксос, Raft) или сущности, над которыми происходит воздействие(транзакции в SI).
...
Например, в RAFT реплика должна отдавать не более одного голоса в пределах одного терма, в противном случае в системе может быть 2 лидера.  Чтобы гарантировать это, перед ответом на RequestVote реплика должна прочитать, за кого она отдала в текущем терме свой голос.
