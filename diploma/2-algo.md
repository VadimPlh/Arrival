...
Для системы или алгоритма разработчики описывают поведение, которое от него стоит ожидать (как ведут себя конкурирующие обращения к объекту и какую последовательность событий мы можем увидеть).

TLA+ позволяет описать содели согласованности у нашей системы/алгоритма. Для проверки этих инвариантов надо промоделировать историю, которую порождает выполнение. Для этого логично использовать tuple, куда надо сохранять события, которые произошли в системе (например действия транзакций, начало и конец вставок в DataStore и т.д.). С помощью этой служебной переменной мы сможем проверять интересующие нас инварианты. Для работы с историей будет удобно написать операторы, чтобы работать с определенным типом событий только
...
В CosmosDB проверяются разные модели согласованности, которые система позволяет установить.
...
### Св-ва системы
Разспределенный алгоритм называется коректным, если он удовлетворяет safety и liveness св-вам

* Safety свойство говорит о том, что если св-во нарушено на траектории, то существует перфикс на котором это св-во нарушается и на любом расширении префикса св-во будет нарушено. Эти св-во должны выполняться в каждо состоянии нашей системы.
* Liveness свойство говорит о том, что для любого бесконечного поведение существует состояние, в котором св-во выполняется.

Liveness св-ва пости никогда не применяются для распределенных систем, так как о завершении можно говорить, только в случае частично синхронной или синхронной сети, а в TLA+ мы не моделируем их.

Любой алгоритм является пересечением свойств безопасности и свойств живучести (теорема https://pron.github.io/posts/tlaplus_part3#safety-and-liveness).

Примеры:
Св-ва соглосованности - это safety свойство. Например, в CosmosDB
...
Пример liveness свойства - это то, что во всех регионах в бд лог будет одинаковый

    Liveness == <>[] (\A i, j \in Regions : Database[i] = Database[j])

В Paxos свойство `SafeValue` - является safety свойством. На каждом шаге или значение еще не выбрано, а если выбрано, то не другое значение никогда не выберется
...
Тоже является safety св-вом, так как это инварианты алгоритма, которые говорят, как работать с конкурентными транзакциями.
