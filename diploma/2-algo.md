...
Percolator - это протокол ckient side расаределенрныз транзакций, который использует BigTable.
Big Table - .это распределденное отказоустойчивое хранилище с которым клиенты взаимодейтсвую. И для них ячейки памяти - это набор атомиков
...
При моделировании обмена сообщениями в нашей системе появляется еще один важный участник - это сама сеть. Сеть может породить проблемы при передаче сообщений между нодами (дубликацию, пропажу и т.д.)
...
Мы будем использовать асинхронную модель сети, так как лучше всего описывает настоящий мир. Но все же мы сильно пессимизируем, когда описываем систему в этой модели, так как в реальности сеть не бывает бесконечно долгой. Это валидно делать, так как система, работающая в таких суровых ограничениях, будет работать и в реальном мире.. Все системы в TLA моделируется в предположении, что сеть асинхронная, так как это наиболее похожая на внешний мир система. В реальном мире ноды могут из-за внешних факторов не отвечать, или долго обрабатывать сообщения.
