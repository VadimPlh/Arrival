...
Чтобы в трейсе для каждого состояния был указан экшен, который был взят при переходе, Next должен представлять собой простую дизъюнкцию:
...
Такой подход требует, что бы мы внесли квантор существования в каждый отдельной экшен, что ухудшает читаемость.

Если же внести все действия одной сущности под один квантор существования, то мы потеряем ссылки на экшены в трейсе:
...
Во время отладки спеки приходится часто запускать model checker. Гораздо удобнее делать это не через IDE, а из командной строки.

Чтобы запустить чекер для спеки Spec.tla, нужно поместить в файл MCSpec.tla значения параметров для чекера и запустить его с помощью команды tla MCSpec.tla
...
* PROPERTIES - произвольные темпоральные формулы
* CONSTANTS - значения параметорв
* SPECIFICATION - спека
