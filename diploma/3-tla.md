...
Понятно, что такой способ линеен от кол-ва состояний (кол-во состояний = exp(кол-во шагов)). Для бесконечных спек мы должны сделать спеку конечной. Этот способ подойдет, когда мы проверяем, что выполняются свойства алгортима.
...
Для поиска "интересных" поведений можно воспольщоваться не систематическим исследованием графа конфигураций, а Simulation mode. В этом методе чекер случайно выбирает траекторию и исследует ее на выполнение свойствам. Этот метод не поможет проверить, что наша спека удовлетворяет всем свойствам, которые мы описали, так как он не проводит полную проверку графа состояний.
