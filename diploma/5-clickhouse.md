...
Для механизма репликации КХ использует ZooKeeper. В нем реплицируется лог. Лог - это последовательность команд (insert, merge, и т.д.). Во время репликации каждая из реплик выполняет этот лог. Реплика скачивает себе данные с других реплик с помощью http.
...
Моделировать системы будет с внешней сороны, то есть такой, какой ее видит польщователь. Основными действующими лицами у нас будут реплики.

    CONSTANTS
        Replicas

Для лучшей читаемсоти кода объеденим действия всех реплик вместе.
