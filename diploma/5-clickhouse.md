...
Описывать системы мы будем с внешней стороны. То есть мы будем моделировать действия реплик и клиентов. Так как алгоритмы, которые мы хотим проверять связаны именно с ними, а не с записями в самом логе. Это похоже на спеку Kafka-и где мы моделируем ноды и их действия. Так же по уровню абстракции можно провести аналогию со спекой Percolator.

Одним из участников является клиент. Он может выполнять Selcet и Insert на реплики. В каждой из спек мы отдельно моделируем действия клиента и объединяем их в один экшен. Аналогично происходит с действиями реплик.

*КОД*

Обеспечиваем недетерминизм тем, что клиент может пойти на любую реплику мы заранее не можем сказать какую реплику он выбирет , для этого мы используем внутри экшена оператор \E для выбора реплики на которую отправил запрос. Так же мы используем конструкцию аналогичную в Экшенах для реплик. Так как в системе мы можем пойти на любую реплику для запроса. И любая реплика может выполнить какое-то действие. Аналогично в спеке Kafka во всех экшенах используется контрукция \E replica \in ... В спеке SI используется аналогичная контрукция. Авторы добавили недетерминизм в том, что может выполнить действие любая из транзакций, так как нет никакого порядка на них.

 *КОД*

 Обмена сообщениями между репликами не происходит. Все связь происходит через ноды в ZooKeeper-е. Аналогию можно провести со спекой Kafka. Там ячейку в ZK считают атомарной ячейкой памяти, где хранится состояние кворума. В спеке КХ мы сделаем аналогично. Все ноды в зукипере мы считаем атомарными. Мы можеим так сделать, так как в реальном КХ все действия с ZK происходят в транзакциях, значит, они происходят атомарно. И мы можем описать изменения состояния в ZK в одном экшене.
...
В сиситеме избегают большую фрагментацию тем, что мерджат блоки с данными. Так как мы хотим, чтобы на всех репликах была однотипная структура мерджа (чтобы было удобно скачивать куски нужные), то история должна быть одинаковая на всех репликах. Для этого существует лидер, который выбирается с помощью ZooKeeper-а. Лидер по своим данным назначает мерджи и пишет информацию о них в лог, потом при выполнении лога реплика выполняет мердж.

### Перевод в КХ
Аналогично спеки Kafka или Percolator - текущий лидер - это просто VARIABLES leader, которая хранится в Znode в зукипере.
