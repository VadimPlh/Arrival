...
* is_active - нода в зукипере
* is_lost - нода в зукипере
* deletedPrefix - удаленный префикс
* logPointers - указатели активных реплик на лог
* log_pointer - указатель на запись в логе, до которой все выполнили(или скопировали к себе в очередь. От очереди мы избавляемся)
* local_log - локальный лог каждой из реплик(набор кусков)

Что мы хотим моделировать:
* log - лог в зукипере
* nexRecordId - id следующей записи, которую мы запищем лог. Мы не моделируем тип сообщения, так как в алоритме очистки лога это несуществено.
* deletedPrefix - префикс, который мы удалили из лога. Для уменьшения кол-ва состояний и простоты спеки мы не будем менять лог в зукипере.
