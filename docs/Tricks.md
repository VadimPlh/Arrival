...
Если мы запустим TLC, то проверим, что у нас сломалось [св-во](https://github.com/fpaxos/fpaxos-tlaplus/blob/c562667ad96bcb9e07a30417a45b49c5d21d1fbe/FPaxos.tla#L89). Вышло это из-за того, что у нас теперь есть ситсема кворумов, которая не пересекается ни с кем из acceptor-ов из одной системы из первой фазы. То есть в алгоритме fpaxos пропозер получил бы выбранное значение и предложил его, а у нас этого не произошло.
